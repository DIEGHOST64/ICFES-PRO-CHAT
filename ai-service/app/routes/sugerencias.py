...
SUGERENCIAS_CACHE_TTL_SECONDS = 300
SUGERENCIAS_CACHE_MAX_ITEMS = 64
...
def _target_general_mix(cantidad: int) -> dict[str, int]:
    ...
    base = total // len(GENERAL_COMPETENCIAS_BASE)
    rem = total % len(GENERAL_COMPETENCIAS_BASE)

    mix = {comp: base for comp in GENERAL_COMPETENCIAS_BASE}
    for idx in range(rem):
        mix[GENERAL_COMPETENCIAS_BASE[idx]] += 1
    return mix
...
def _apply_block_structure(items: list[dict], competencia_label: str) -> list[dict]:
    ...
    while cursor < len(items):
        ...
        base_candidates = [str(row.get("texto_base", "")).strip() for row in block_items if str(row.get("texto_base", "")).strip()]
        canonical_base = max(base_candidates, key=len) if base_candidates else ""
...
def _compose_fragment_window(docs: list[str], idx: int) -> str:
    ...
    if starts_cut and idx > 0:
        ...
        if prev_txt:
            pieces.insert(0, prev_txt[-420:])

    if ends_cut and idx + 1 < len(docs):
        next_txt = re.sub(r"\s+", " ", str(docs[idx + 1] or "")).strip()
        if next_txt:
            pieces.append(next_txt[:420])

    merged = re.sub(r"\s+", " ", " ".join(pieces)).strip()
    return _repair_text_boundaries(merged, max_len=1800)


def _prepare_fragments(docs: list[str], indices: list[int], min_len: int = 120) -> list[str]:
    cleaned: list[str] = []
    seen: set[str] = set()
...
def _normalize_generated_questions(
    ...
    for item in raw:
        ...
        texto_base = _normalize_text_base_quality(str(item.get("texto_base", "")).strip())
        ...
        if enunciado.lower().startswith("pregunta") and len(enunciado) < 40:
            continue
        if texto_base.lower().startswith("fragmento de entrenamiento"):
            continue
        if not texto_base:
            continue
...
def _target_english_mix(cantidad: int) -> dict[str, int]:
    ...
    if cantidad <= 0:
        return {"reading": 0, "vocabulary": 0, "grammar": 0}
    ...
    base = {k: int(cantidad * w) for k, w in weights.items()}
    assigned = sum(base.values())

    order = ["reading", "vocabulary", "grammar"]
    idx = 0
    while assigned < cantidad:
        base[order[idx % len(order)]] += 1
        assigned += 1
        idx += 1

    if cantidad >= 5 and base["grammar"] == 0:
        for donor in ("reading", "vocabulary"):
            if base[donor] > 1:
                base[donor] -= 1
                base["grammar"] += 1
                break
...
def _balance_english_questions(raw: list[dict], cantidad: int) -> list[dict]:
    """Rebalancea preguntas para asegurar mezcla reading/vocabulary/grammar."""
    ...
    selected: list[dict] = []
    for t in ("reading", "vocabulary", "grammar"):
        needed = targets[t]
        take = pools[t][:needed]
        selected.extend(take)
        pools[t] = pools[t][needed:]

    leftovers = pools["reading"] + pools["vocabulary"] + pools["grammar"] + others
    if len(selected) < cantidad:
        selected.extend(leftovers[:cantidad - len(selected)])
...
def _generate_questions_sync(fragments: list[str], cantidad: int,
    ...
    if is_english:
        ...
        extra_rules = (
            ...
            "- Construye una mezcla balanceada de tipos de pregunta:\n"
            "  1) Reading comprehension (texto corto + inferencia/idea principal)\n"
            "  2) Vocabulary in context (sentido de palabra/frase dentro del texto)\n"
            "  3) Language use / grammar in context (uso correcto en contexto real)\n"
            f"- Para esta tanda de {cantidad} preguntas, objetivo de mezcla: reading={mix['reading']}, vocabulary={mix['vocabulary']}, grammar={mix['grammar']}.\n"
            f"- Nivel objetivo principal para esta tanda: {target_level}.\n"
            "- Al menos 80% de las preguntas deben coincidir con el nivel objetivo y el resto puede variar para mantener progresion.\n"
            "- Todo enunciado y opciones deben estar en inglés natural (nivel A2-B1).\n"
            "- texto_base debe estar en ingles. Si el fragmento fuente esta en espanol, crea una version fiel en ingles\n"
            "  conservando el mismo significado y datos clave para resolver la pregunta.\n"
            "- Evita preguntas puramente memorísticas o de traducción literal.\n"
            "- Las distractoras deben ser plausibles y cercanas semánticamente.\n"
            "- La respuesta correcta debe estar explícitamente sustentada por texto_base.\n"
            "- Cada pregunta debe incluir tipo_ingles con uno de estos valores: reading | vocabulary | grammar.\n"
            "- Cada pregunta debe incluir nivel_cefr con uno de estos valores: A2 | B1.\n"
            "- Si usas expresiones como 'according to the graph/chart/figure/table', DEBES incluir en texto_base\n"
            "  la informacion de esa grafica/tablas en formato textual claro (datos resumidos), para que sea resoluble.\n"
            "- Si no hay datos visuales claros en el fragmento, NO menciones graph/chart/figure/table en el enunciado.\n"
            "- En explicacion, justifica en español en 1-2 oraciones por qué la opción correcta es la mejor.\n"
        ...
        extra_rules = (
            ...
            "- En las preguntas de Inglés, incluir tipo_ingles (reading|vocabulary|grammar) y nivel_cefr (A2|B1).\n"
    ...
    prompt = (
        ...
        f"- NO uses fragmentos administrativos/legales/editoriales (p.ej., términos y condiciones, derechos de autor,"
        f"  licencias, prohibiciones de reproducción, encabezados de instrucciones) para construir preguntas.\n"
        f"- Si un fragmento es legal/no académico, ignóralo y usa otro fragmento con contenido conceptual o aplicado.\n"
        f"- El enunciado DEBE estar alineado con texto_base: comparte términos clave y contexto temático real.\n"
        f"- Está PROHIBIDO mezclar temas (ej.: texto sobre ética animal y pregunta sobre pistas de aeropuerto).\n"
        f"- Antes de devolver cada pregunta, verifica internamente coherencia semántica entre texto_base, enunciado y respuesta correcta.\n"
        f"- Si la competencia requiere fórmulas/cálculos, usa notación LaTeX en enunciado/opciones con delimitadores \\( ... \\) o $$ ... $$.\n"
        f"- No fuerces LaTeX en preguntas de lectura o ciudadanía si no aporta claridad.\n"
        f"- texto_base: debe ser un pasaje completo y coherente (aprox. 70 a 180 palabras) que el estudiante NECESITA "
        f"leer para resolver el bloque de preguntas; evita frases aisladas o incompletas.\n"
...
async def sugerencias(
    ...
    preguntas_curadas = _get_curated_bank_questions(
    ...
    if len(preguntas_curadas) >= cantidad:
        preguntas_curadas = preguntas_curadas[:cantidad]
        _set_cached_sugerencias(cache_key, preguntas_curadas)
        return preguntas_curadas

    preguntas_seed = preguntas_curadas[:cantidad] if preguntas_curadas else []

    # Recuperar fragmentos de contexto priorizando separación por competencia y usando
    # tanto cuadernillos de práctica como de ejemplos explicados.
    ...
    if entrenamiento_general:
        ...
        for comp_name in GENERAL_COMPETENCIAS_BASE:
            ...
            fragments = _prepare_fragments(comp_docs, selected_indices)
            if not fragments:
        ...
        for batch_size in _build_batch_plan(cantidad, max_per_batch=10):
            sample_size = min(len(docs), max(batch_size * 3, 10))
            indices = random.sample(range(len(docs)), sample_size)
            fragments = _prepare_fragments(docs, indices)
            if not fragments:
